...
ホスト PC との通信は UART テキストプロトコル（PI/VE/SB/SW/MO/PA/BO/EB/EW/ED）。
詳細は [`etc/protocol.md`](etc/protocol.md)。リファレンス実装は
[`verif/golden/reversi_rules.py`](verif/golden/reversi_rules.py) が **唯一の golden** で、
Verilog の挙動が Python と異なれば **Verilog を直す**。
...
- **シナリオ回帰を最初から維持** — `verif/golden/scenarios/*.txt` 形式の回帰を MVP 段階から回す。
